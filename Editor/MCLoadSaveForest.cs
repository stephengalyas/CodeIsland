...
public class MCLoadSaveForest: MonoBehaviour {
    ...
    public static bool LoadProgress(bool skip = false, bool testing = false)
    {
        ...
        else // Not testing.
        {
            ...
            //try
            //{
            //    XmlNodeList nodes = xmlDoc.SelectSingleNode("/config/progress").ChildNodes; // Get the child nodes with the character's progress.

            //    // Load the data.
            //    MCGodScriptBeach.completed = Convert.ToBoolean(nodes[0].InnerText);
            //    MCGodScriptBeach.gobAtPlayer = Convert.ToBoolean(nodes[1].InnerText);
            //    MCGodScriptBeach.dialogueBoxFinished = Convert.ToBoolean(nodes[2].InnerText);
            //    MCGodScriptBeach.gobOffMap = Convert.ToBoolean(nodes[3].InnerText);
            //    MCGodScriptBeach.playerAtCave = Convert.ToBoolean(nodes[4].InnerText);

            //    if (testing == true) // Testing. Display status.
            //    {
            //        EditorUtility.DisplayDialog("Load Progress Bools", "Success", "Play Game");
            //    }
            //} // Close try block.
            //catch (XmlException exc) // Error loading data from config file.
            //{
            //    if (testing == true)
            //    {
            //        EditorUtility.DisplayDialog("ERROR", "Could not load level progress. Error message: \n\n" + exc.Message, "Load Defaults"); // Display message.
            //    }
    ...
    public static bool SaveProgress(bool skip = false, bool testing = false)
    {
        ...
        else // Save progress to the main character's configuration file.
        {
            ...
            //try
            //{
            //    xmlDoc.SelectSingleNode("/config/progress/complete").InnerText = MCGodScriptBeach.completed.ToString();
            //    xmlDoc.SelectSingleNode("/config/progress/gobMoveToMC").InnerText = MCGodScriptBeach.gobAtPlayer.ToString();
            //    xmlDoc.SelectSingleNode("/config/progress/gobDialogueComp").InnerText = MCGodScriptBeach.dialogueBoxFinished.ToString();
            //    xmlDoc.SelectSingleNode("/config/progress/gobLeaveScreen").InnerText = MCGodScriptBeach.gobOffMap.ToString();
            //    xmlDoc.SelectSingleNode("/config/progress/MCToCave").InnerText = MCGodScriptBeach.playerAtCave.ToString();

                xmlDoc.Save(path);

            //    if (testing == true) // Testing. Display progress.
            //    {
            //        EditorUtility.DisplayDialog("Save Progress Bools", "Success", "Next Level");
            //    }
            //} // Close try block.
            //catch (Exception exc) // Error saving data to config file.
            //{
            //    if (testing == true)
            //    {
            //        EditorUtility.DisplayDialog("ERROR", "Could not save progress booleans. Error message: \n\n" + exc.Message, "Load Defaults"); // Display message.
            //    }
            //    else // Not testing. Display user-friendly error message.
            //    {
            //        EditorUtility.DisplayDialog("Could Not Save Progress", "Your progress could not be saved." +
